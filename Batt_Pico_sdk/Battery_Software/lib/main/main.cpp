...
{
    burnStatus.has_burned_before = false;
    satellite.bit_set(STATUS_REG, PRIOR_BURN_ATTEMPT, false);
    satellite.burned = false;
    burnStatus.previous_brownout = false;
...
BurnStatus checkBurnStatus(pysquared &satellite, tools &t, uint8_t *data)
{
    ...
    else if ((data[STATUS_REG] & (1 << BROWNOUT_BIT)) == (1 << BROWNOUT_BIT))
    {
